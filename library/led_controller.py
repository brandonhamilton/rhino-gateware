...
class LedController(GPIO):
	def __init__(self, baseapp, count, csr_name="led_controller", led_name="user_led"):
		signals = [(baseapp.mplat.request(led_name), OUTPUT, "led"+str(i))
			for i in range(count)]
		GPIO.__init__(self, baseapp, csr_name, UID_LED_CONTROLLER, signals)

class LedBlinker:
	def __init__(self, baseapp, divbits=26, led_name="user_led"):
		self._led = baseapp.mplat.request(led_name)
